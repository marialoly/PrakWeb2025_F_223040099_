<div class="relative overflow-x-auto bg-neutral-primary-soft shadow-xs rounded-base border border-default">
            ...
            <div class="relative">
                <div class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                    <svg class="w-4 h-4 text-body" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-width="2" d="m21 21-3.5-3.5M17 18a7 7 0 1 1-14 0 7 7 6 6 1 14 0Z"/>
                    </svg>
                </div>
    ...
    <tr class="bg-white border-b hover:bg-gray-50 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-600">
        ...
        <td class="px-6 py-4 flex items-center gap-3">
            <a href="{{ route('dashboard.show', $post->slug) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">View</a>
            
            <a href="{{ route('dashboard.edit', $post->slug) }}" class="font-medium text-yellow-600 dark:text-yellow-500 hover:underline">Edit</a>

            <form action="{{ route('dashboard.destroy', $post->slug) }}" method="POST" class="inline">
                @csrf
                @method('delete')
                <button type="submit" class="font-medium text-red-600 dark:text-red-500 hover:underline border-none bg-transparent cursor-pointer" onclick="return confirm('Yakin mau hapus postingan ini?')">
                    Delete
                </button>
            </form>
        </td>
        ...
        <nav aria-label="Page navigation">
            <ul class="flex space-x-px text-sm">
                ...
                @foreach($posts->getUrlRange(1, $posts->lastPage()) as $page => $url)
                    ...
                    <li>
                        <a href="{{ $url }}" aria-current="page" class="flex items-center justify-center text-brand bg-neutral-tertiary-medium box-border border border-default-medium hover:text-brand font-medium text-sm w-10 h-10 focus:outline-none">{{ $page }}</a>
                    </li>
                    @else
                    <li>
