        ...
        <header class="mb-8">
            ...
            @if($post->image)
                <div style="max-height: 400px; overflow: hidden;">
                    <img src="{{ asset('storage/' . $post->image) }}" alt="{{ $post->title }}" class="w-full object-cover rounded-lg mb-6">
                </div>
            @else
                <img src="https://source.unsplash.com/1200x400?{{ $post->category->name }}" alt="Random Image" class="w-full h-64 object-cover rounded-lg mb-6">
            @endif

        </header>

        <div class="prose prose-lg max-w-none">
            <p class="text-xl text-gray-600 mb-6 font-semibold">{{ $post->excerpt }}</p>
